...
def plot_solution_fd(u, X, Y, u_exact, title_prefix="Finite Difference"):
    ...
    # Numerical solution
    im1 = axes[0,0].contourf(X, Y, u, levels=50, cmap='viridis')
    ...
    # Exact solution
    im2 = axes[0,1].contourf(X, Y, u_exact, levels=50, cmap='viridis')
    ...
    # Error
    error = u - u_exact
    im3 = axes[1,0].contourf(X, Y, error, levels=50, cmap='RdBu_r')
    ...
    mid_idx = X.shape[0] // 2
    axes[1,1].plot(X[mid_idx, :], u[mid_idx, :], 'b-', label='Numerical', linewidth=2)
    axes[1,1].plot(X[mid_idx, :], u_exact[mid_idx, :], 'r--', label='Exact', linewidth=2)
    axes[1,1].set_title(f'Cross-section at y = {Y[mid_idx, 0]:.2f}')
    ...
    plt.savefig(f"{title_prefix}_solution.png", dpi=300)
    plt.close(fig)
